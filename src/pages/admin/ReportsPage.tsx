...
import { useOrgReportData, useOrgLeaveRequests, useOrgPermissionRequests } from '@/hooks/useAdminReports';
...
const statusLabels: Record<RequestStatus, string> = { draft: 'Draft', pending: 'Pending', approved: 'Approved', rejected: 'Rejected', cancelled: 'Cancelled' };
...
function attendanceReport(rows: TeamAttendanceRow[]) {
  const columns: ColumnDef<TeamAttendanceRow, any>[] = [
...
function leaveReport(rows: TeamLeaveRequestRow[]) {
  const columns: ColumnDef<TeamLeaveRequestRow, any>[] = [
    { header: 'Employee', accessorKey: 'employeeName' },
    { header: 'Code', accessorKey: 'employeeCode' },
...
function permissionReport(rows: TeamPermissionRow[]) {
  const columns: ColumnDef<TeamPermissionRow, any>[] = [
    { header: 'Employee', accessorKey: 'employeeName' },
    { header: 'Code', accessorKey: 'employeeCode' },
...
export default function ReportsPage() {
  ...
  const { data: attendanceRows, isLoading: attendanceLoading } = useOrgReportData(rangeStart, rangeEnd);
  const { data: leaveRows, isLoading: leaveLoading } = useOrgLeaveRequests();
  const { data: permissionRows, isLoading: permissionLoading } = useOrgPermissionRequests(rangeStart, rangeEnd);

  const employeeOptions = useMemo(
    () => [{ value: 'all', label: 'All' }, ...(employees ?? []).map((e) => ({ value: e.id, label: e.employeeName }))],
    [employees],
  );
  ...
  const filenameBase = `org-${reportType}-${appliedRange.from}-to-${appliedRange.to}`;

  return (
    <div>
      <Breadcrumb items={[{ label: 'Administration' }, { label: 'Reports' }]} />
        ...
        <FilterBar
          actions={
            <>
              ...
              <FilterButton onClick={() => exportToPdf(report.exportColumns as ExportColumn<unknown>[], report.rows, `${filenameBase}.pdf`, reportLabel)}>
                <span className="flex items-center gap-1.5">
